...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
		if ($_GET['id'])
		{
			...
			TPL::assign('topic_info',$this->model('topic')->get_topic_by_id($project_info['topic_id']));
			//wll -end
		...
		else
		{
			...
			if ($project_list = $this->model('project')->get_projects_list($_GET['category'], 1, 'ONLINE', $_GET['page'], 9, $order_by,$_GET['topic']))
			{
			...
			if ($_GET['topic']) {
				TPL::assign('content_nav_menu', $this->model('menu')->get_nav_menu_list());
				TPL::assign('topic_info',$this->model('topic')->get_topic_by_id($_GET['topic']));
				TPL::output('project/topicSquare');
			}else{
